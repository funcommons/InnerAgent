...
request.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === HTTP_STATUS.UNAUTHORIZED && userStoreGetter) {
      ...
      if (store.hasToken) store.clearAuth()
      if (!(originalConfig as RequestConfig | undefined)?.skipAuthRedirect) {
        redirectToLogin()
      }
    }
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case HTTP_STATUS.UNAUTHORIZED:
          ...
          if (!(silent === true && (error.config as RequestConfig | undefined)?.skipAuthRedirect)) {
            redirectToLogin()
          }
          break
...
export interface RequestConfig extends AxiosRequestConfig {
  ...
  silent?: boolean
  /** 跳过 401 自动跳登录（后台静默请求，由调用方呈现错误，如 embed-token 拉取） */
