/**
 * ACME DEMO 被嵌页引导 — mountIframeAgent(P4/W15 postMessage 桥)。
 *
...
async function bootstrap() {
  const { mountIframeAgent } = await import('./iframe-child.js')

  // allowedParentOrigins: 允许嵌入本页的宿主来源 allowlist(生产按部署域配置;
  // 缺省同源)。本 DEMO 宿主页与被嵌页同源(同一 vite dev 服务)。
  mountIframeAgent({
...
bootstrap().catch((error) => {
  console.error('[acme-ia-frame] 引导失败:', error)
})
