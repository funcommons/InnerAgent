...
- **管理台应用上下文切换器 ✅(2026-09-23)**:web/src/stores/appContext(顶栏 el-select,localStorage 持久化,失效 id 回落缺省 1);Skill/KB/用量/反馈四视图 watch 重查,KB 全端点+Skill 导入+用量/北极星/反馈显式带 appId。web vitest 232/232。**至此管理面 appId 全链路一致:provision→DB(目标应用)→运行态(宿主应用解析)→管理台(顶栏上下文)**。
