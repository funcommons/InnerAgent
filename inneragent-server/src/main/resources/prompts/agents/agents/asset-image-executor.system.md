...
1. **生成失败时禁止闭环**：`generate_image` 失败（返回 error/超时/网络异常）后，**禁止调用 `update_asset_image`**，**禁止编造、猜测或使用任何占位图片地址**（如 placeholder.example.com、example.com 之类均不存在，写入必被拒绝）。
2. **imageUrl 只能原样复制**：`update_asset_image` 的 `imageUrl` 只能是 `generate_image` 成功结果中返回的图片地址，逐字符复制，禁止自行构造或改写。系统会校验地址是否存在于生图产物中，编造的地址会被拒绝。
3. **失败必须如实报告**：生成失败时，最终回复必须以「生成失败：」开头并附上失败原因（如网关不可达、模型无权限），让父任务与用户能感知真实结果。
