#!/usr/bin/env zsh
# P1-T3c 冒烟:建会话 → SSE 出流 → 断开 → Last-Event-ID 重连续流(开发计划 §3.4)
# 依赖:P0-T5 的演示 Agent(agentType=demo)+ mock 模型(V4__demo_seed.sql)+
#       匿名演示头(X-IA-Demo-User);BASE 默认本机 18090。
# 契约:P1-T3b API 收口(02-技术方案 §7.1 ADR-T4)——
#   POST /ia/api/v1/runs(SSE)、GET /ia/api/v1/runs/{runId}/events(Last-Event-ID 重连)。
# 请求体字段以 AiChatReqVO 为准:message/agentType/toolExecutionMode/enabledSkills
#   (+ 可选 context{page,object},落运行上下文)。
...
curl -sN -X POST "$BASE/ia/api/v1/runs" \
  ...
  -d '{"conversationId":null,"message":"现在几点了?","agentType":"demo","toolExecutionMode":"DEFAULT","enabledSkills":[],"context":{"page":{"id":"smoke-page"},"object":{"type":"doc"}}}' \
  | tee "$JAR" &
CURL_PID=$!
...
echo "[2] runId=$RUN_ID lastEventId=$LAST_ID;断开后重连(仅 Last-Event-ID 头)…"
curl -sN "$BASE/ia/api/v1/runs/$RUN_ID/events" \
