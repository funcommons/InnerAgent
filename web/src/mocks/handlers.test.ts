...
describe('mock 后端:mini 知识库(P4-W14 契约形)', () => {
  ...
  it('import:文本导入(标题/来源/分块参数);校验 400 语义齐全', async () => {
    const doc = await kbAdminApi.importDocument(1, { title: '接入指南', source: 'upload', content: 'a'.repeat(1200), chunkSize: 500, chunkOverlap: 50 })
    expect(doc.status).toBe('active')
    expect(doc.chunkCount).toBe(3) // ceil(1200/500)
    await expect(kbAdminApi.importDocument(1, { title: '', content: 'x' })).rejects.toMatchObject({ status: 400 })
    await expect(kbAdminApi.importDocument(1, { title: 't', content: '  ' })).rejects.toMatchObject({ status: 400 })
    await expect(kbAdminApi.importDocument(1, { title: 't', content: 'c', chunkSize: 100, chunkOverlap: 100 }))
      .rejects.toMatchObject({ status: 400, message: expect.stringContaining('chunkOverlap') })
    await expect(kbAdminApi.importDocument(1, { title: 't', content: 'c', metadata: '{bad' }))
      .rejects.toMatchObject({ status: 400, message: expect.stringContaining('metadata') })
    await expect(kbAdminApi.importDocument(1, { title: 't', content: 'c', metadata: '[1]' }))
      .rejects.toMatchObject({ status: 400, message: expect.stringContaining('JSON 对象') })
  })

  it('search:检索调试(top-k/降级标记);空查询 → simple 降级空命中', async () => {
    const result = await kbAdminApi.search(1, { q: '员工', topK: 2 })
    ...
    const degraded = await kbAdminApi.search(1, { q: '' })
  ...
  it('状态门控/inactive 不参与检索;rebuild-index;更新重分块;删除', async () => {
    const doc = await kbAdminApi.importDocument(1, { title: '门控文档', content: 'x' })
    // deactivate → 检索不再命中
    const deactivated = await kbAdminApi.deactivate(1, doc.id)
    expect(deactivated.status).toBe('inactive')
    expect(deactivated.active).toBe(false)
    const afterOff = await kbAdminApi.search(1, { q: '门控文档' })
    expect(afterOff.hits.find(h => h.documentId === doc.id)).toBeUndefined()
    // activate 恢复
    const activated = await kbAdminApi.activate(1, doc.id)
    expect(activated.active).toBe(true)
    // 重建索引(回写行)
    const rebuilt = await kbAdminApi.rebuildIndex(1, doc.id)
    expect(rebuilt.id).toBe(doc.id)
    // 更新带 content → 重分块
    const updated = await kbAdminApi.update(1, doc.id, { title: '门控文档(改)', content: 'b'.repeat(900), chunkSize: 400 })
    expect(updated.title).toContain('改')
    expect(updated.chunkCount).toBe(3) // ceil(900/400)
    await kbAdminApi.remove(1, doc.id)
    await expect(kbAdminApi.get(1, doc.id)).rejects.toMatchObject({ status: 404 })
  })
})
