...
describe('webhooks store', () => {
  ...
  it('loadDeliveries 过滤失败投递;simulateFailure 追加可重试投递', async () => {
    const store = useWebhooksStore()
    ...
    await store.loadDeliveries()
    const before = store.deliveriesTotal
    const d = await store.simulateFailure('run.cancelled', 'run-999')
    expect(d.success).toBe(false)
    expect(d.maxAttempts).toBe(5)
    expect(store.deliveriesTotal).toBe(before + 1)
  })
