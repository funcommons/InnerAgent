...
3. **查询项目画风与画幅**：调用 `get_project(projectId)` 提取 `artStyleInfo` 的 `description`、`imagePrompt` 和 `referenceImageUrl`，并记录顶层 `aspectRatio`（项目画面比例，如 `9:16`；可能缺失）。
...
7. **调用生图**：调用 `generate_image` 生成一张图片。**必须传 `aspectRatio`＝第 3 步记录的项目画面比例**（如 `9:16`），保证首尾帧与项目画幅一致；项目未配置比例时不传该参数，不得自行猜测。
